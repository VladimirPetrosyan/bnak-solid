  ...
  return (
    <header style="position:sticky;top:0;z-index:60;background:#fff;box-shadow:0 1px 0 #ebeae7">
      <div style="width:100%;max-width:1400px;margin:0 auto;padding:12px clamp(16px,3vw,28px);display:flex;align-items:center;gap:16px;flex-wrap:wrap">
        <div onClick={() => go('search')} style="display:flex;align-items:center;gap:8px;cursor:pointer;flex:0 0 auto">
              ...
              {(key) => {
                ...
                return (
                  <div
                    ...
                    style={`padding:8px 16px;border-radius:999px;white-space:nowrap;font-size:14px;font-weight:600;cursor:pointer;background:${on() ? TEAL_T : 'transparent'};color:${on() ? TEAL_TX : MUTED}`}
                  >
                    {t()[key]}
                  </div>
                );
        ...
        <div style="display:flex;align-items:center;gap:8px;flex:0 0 auto;margin-left:auto">
              ...
              {(code) => {
                ...
                return (
                  <div
                    ...
                    style={`padding:4px 8px;border-radius:999px;font-size:12px;font-weight:700;cursor:pointer;background:${on() ? '#fff' : 'transparent'};color:${on() ? INK : MUTED};box-shadow:${on() ? '0 1px 3px rgba(28,27,25,.14)' : 'none'}`}
                  >
                    {code}
                  </div>
                );
          ...
          <Show when={!state.isMob}>
            <div
              onClick={guard('fav')}
              title={t().favs}
              style="position:relative;width:40px;height:40px;border-radius:12px;display:flex;align-items:center;justify-content:center;cursor:pointer"
            >
          ...
          <div onClick={guard('cabinet')} style={cabinetPillStyle(state.isMob)}>
            <span
          ...
          <div
            onClick={guard('post')}
            style="display:flex;align-items:center;gap:8px;height:40px;padding:0 16px;border-radius:12px;background:#0e7c73;color:#fff;font-size:14px;font-weight:700;cursor:pointer;box-shadow:0 6px 16px -8px rgba(14,124,115,.6)"
          >
