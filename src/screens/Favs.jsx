...
export default function Favs() {
  ...
  return (
    <div style="width:100%;max-width:1120px;margin:0 auto;padding:30px clamp(14px,3vw,28px) 48px;animation:bnIn .2s ease">
      ...
      <div style="font-size:15px;color:#6f6d68;max-width:60ch">{t().favSub}</div>
      ...
      <Show when={hasAlert()}>
        <div style="margin-top:22px;display:flex;gap:14px;align-items:flex-start;padding:18px 20px;border-radius:16px;background:#fceeeb">
          ...
          <div style="min-width:0">
            <div style="font-size:16px;font-weight:800;color:#93331f">{t().favAlertTitle}</div>
            <div style="font-size:14px;line-height:1.5;color:#93331f;margin-top:3px">{t().favAlertText}</div>
          </div>
        </div>
      </Show>
              ...
              <div class="bn-fav-card" style={`opacity:${archived() ? 0.62 : 1}`}>
                ...
                <button type="button" onClick={() => openListing(l.id)} class="bn-fav-info" style="text-align:left">
                  <div style="display:flex;align-items:baseline;gap:10px;flex-wrap:wrap">
                    <span
                      style={`font-size:20px;font-weight:800;letter-spacing:-.02em;text-decoration:${archived() ? 'line-through' : 'none'};color:${archived() ? FAINT : INK}`}
                    >
                      {c().price}
                    </span>
                ...
                <div class="bn-fav-status">
                  <span
                    style={`display:inline-flex;align-items:center;gap:7px;padding:7px 12px;border-radius:999px;font-size:12px;font-weight:700;background:${archived() ? SOFT : TEAL_T};color:${archived() ? FAINT : TEAL_TX}`}
                  >
