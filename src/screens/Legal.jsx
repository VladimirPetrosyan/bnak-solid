  ...
  return (
    ...
    >
      ...
      <div style="flex:1 1 auto;min-width:0">
        <div style="font-size:16px;font-weight:700">{t()[LEGAL_KEY_BY_ID[props.id]]}</div>
      ...
      <Show when={props.notFound}>
        <div style="margin-bottom:20px;display:flex;gap:14px;align-items:flex-start;padding:18px 20px;border-radius:16px;background:#fceeeb">
          ...
          <div style="min-width:0">
            <div style="font-size:16px;font-weight:800;color:#93331f">{t().legalNotFoundTitle}</div>
            <div style="font-size:14px;line-height:1.5;color:#93331f;margin-top:3px">{t().legalNotFoundText}</div>
          </div>
        </div>
      </Show>

      <h1 style="margin:0 0 6px;font-size:clamp(24px,4vw,30px);font-weight:800;letter-spacing:-.03em">{t().legalTitle}</h1>
      <div style="font-size:15px;color:#6f6d68;max-width:60ch">{t().legalListSub}</div>

      <div
        style={`margin-top:18px;padding:14px 16px;border-radius:14px;background:${SOFT};font-size:13px;line-height:1.55;color:${MUTED}`}
      >
        {t().legalBetaNote}
      </div>
        ...
        {(d) => (
          <>
            ...
            <nav style="margin-top:20px;padding:16px 18px;border-radius:16px;background:#fff;box-shadow:0 1px 2px rgba(28,27,25,.05)">
              <div style="font-size:12px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:#9a9793">
                {t().legalTitle}
              </div>
              <div style="margin-top:10px;display:flex;flex-direction:column;gap:7px">
                <For each={d().sections}>
                  {(s, i) => (
                    <a href={`#legal-sec-${i()}`} style={`font-size:14px;color:${TEAL};text-decoration:none`}>
                      {s.heading}
                    </a>
                  )}
                </For>
            ...
            <div style="margin-top:20px;display:flex;flex-direction:column;gap:16px">
              <For each={d().sections}>
                {(s, i) => (
                  ...
                  >
                    <h2 style={`margin:0 0 8px;font-size:16px;font-weight:800;letter-spacing:-.01em;color:${INK}`}>{s.heading}</h2>
