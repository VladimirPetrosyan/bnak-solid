      ...
      <div style="margin-top:20px;background:#fff;border-radius:18px;padding:24px;box-shadow:0 1px 2px rgba(28,27,25,.05)">
        <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
          <div style="flex:1 1 240px">
            <div style="font-size:16px;font-weight:800">{t().savedSearchT}</div>
          ...
          <button
            ...
            class="bn-tap"
            onClick={saveSearch}
            style="padding:12px 16px;border-radius:12px;background:#f2f1ee;font-size:14px;font-weight:700"
          >
            {t().saveSearch}
          </button>
        </div>
            ...
            {(q, i) => (
              <div style="display:flex;align-items:center;gap:12px;padding:12px 16px;border-radius:13px;background:#f7f7f6">
                ...
                <button
                  ...
                  style="font-size:13px;font-weight:700;color:#0e7c73"
                >
                  {t().applyW}
                </button>
                <button
                  ...
                  style="font-size:13px;color:#9a9793"
                >
                  {t().removeW}
                </button>
              </div>
          ...
          {(l) => {
            ...
            return (
              ...
              >
                ...
                <button type="button" onClick={() => openListing(l.id)} style="flex:1 1 220px;min-width:0;text-align:left">
                  <div style="font-size:20px;font-weight:800;letter-spacing:-.02em">{c().price}</div>
                  <div style="font-size:14px;font-weight:700;margin-top:4px">{c().title}</div>
                  <div style="font-size:13px;color:#6f6d68;margin-top:4px">{c().addr}</div>
                </button>
                <span
                  style={`display:inline-flex;align-items:center;padding:8px 12px;border-radius:999px;font-size:12px;font-weight:700;background:${archived() ? SOFT : TEAL_T};color:${archived() ? FAINT : TEAL_TX}`}
                >
